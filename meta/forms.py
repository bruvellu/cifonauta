...
class UploadMediaForm(forms.ModelForm, SendEmailForm):
    ...
    class Meta:
        ...
        widgets = {
            "taxa": forms.SelectMultiple(
                attrs={"class": "select2-taxons", "multiple": "multiple"}
            ),
            "authors": forms.SelectMultiple(
                attrs={"class": "select2-authors", "multiple": "multiple"}
            ),
            "references": forms.SelectMultiple(
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["taxa"].queryset = self.fields["taxa"].queryset.exclude(
            name="Sem táxon"
        )
    ...
    def clean_terms(self):
        ...
        if not terms:
            self.add_error(
                "terms",
                forms.ValidationError(
                    _("Você precisa aceitar os termos"), code="required"
                ),
            )
...
class MyMediaForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {"taxons": forms.CheckboxSelectMultiple()}
    ...
    class Meta:
        ...
        widgets = {
            ...
            "is_public": forms.CheckboxInput(
                attrs={"class": "dashboard-input is-public-checkbox"}
            ),
        }
...
class BatchActionsForm(forms.ModelForm, SendEmailForm):
    ...
    status_action = forms.ChoiceField(
        label=_("Status"), choices=STATUS_CHOICES, initial="maintain"
    )
    title_pt_br_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "title_pt_br_action"}),
    )
    title_en_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "title_en_action"}),
    )
    caption_pt_br_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "caption_pt_br_action"}),
    )
    caption_en_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "caption_en_action"}),
    )
    authors_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "authors_action"}),
    )
    date_created_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "date_created_action"}),
    )
    taxa_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "taxa_action"}),
    )
    tags_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "tags_action"}),
    )
    scale_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "scale_action"}),
    )
    references_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "references_action"}),
    )
    country_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "country_action"}),
    )
    state_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "state_action"}),
    )
    city_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "city_action"}),
    )
    location_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "location_action"}),
    )
    latitude_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "latitude_action"}),
    )
    longitude_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "longitude_action"}),
    )
    license_action = forms.ChoiceField(
        choices=ACTION_CHOICES,
        initial="maintain",
        widget=forms.Select(attrs={"data-field-action": "license_action"}),
    )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "tags": CustomCheckboxSelectMultiple(
                attrs={"class": "flex-direction--column"}
            ),
            "references": forms.SelectMultiple(
                attrs={"class": "select2-references", "multiple": "multiple"}
            ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name in self.fields:
            self.fields[
                field_name
            ].required = False  # If there is a required field and I delete it from cleaned_data in the clean method, it doesn't work, this is why I make all of the fields not required
        ...
        if self.view_name == "my_media_list":
            ...
            or self.view_name == "my_curations_media_list"
        ):
            ...
            if self.view_name == "editing_media_list":
                self.fields["status_action"].choices = (
                    self.STATUS_CHOICES[0],
                    self.STATUS_CHOICES[1],
                )
            elif self.view_name == "revision_media_list":
                self.fields["status_action"].choices = (
                    self.STATUS_CHOICES[0],
                    self.STATUS_CHOICES[2],
                )
            else:
                self.fields.pop("status_action", None)
                self.fields.pop("status", None)
            ...
            if self.cleaned_data["authors"] != "maintain":
                if author_person not in self.cleaned_data["authors"]:
                    raise ValidationError(
                        f"Você ({media_instance.user}) deve ser incluído como autor"
                    )
            ...
            if action_field in self.cleaned_data:
                if self.cleaned_data[action_field] != "maintain":
                    if not self.cleaned_data[field]:
                        raise ValidationError(
                            f"O campo {self.fields[field].label} é obrigatório"
                        )
...
class DashboardFilterForm(forms.Form):
    ...
    status = forms.MultipleChoiceField(
        ...
        widget=forms.SelectMultiple(
            attrs={"class": "select2-status", "multiple": "multiple"}
        ),
    )

    alphabetical_order = forms.BooleanField(
        required=False,
        initial=False,
...
class DisplayForm(forms.Form):
    ...
    n = forms.ChoiceField(
        required=False, choices=ITEMS, initial="40", label=_("Arquivos por página")
    )
    ...
    order = forms.ChoiceField(
        required=False, choices=ORDER, initial="desc", label=_("Ordem")
    )
