...
class Taxon(MPTTModel):
    ...
    def update_taxonomic_tree(self):
        RANKS_NAMES = {
                    'kingdom': 'Reino',
                    'phylum': 'Filo',
                    'class': 'Classe',
        ...
        if len(records)==0:
            ...
            curadory.taxons.add(self)
            self.parent = Taxon.objects.get_or_create(name="Sem Táxon")[0]
            self.is_valid = False
        else:
            for rank in RANKS_NAMES.keys():
                if rank != records['rank'].lower():
                    ...
                    with Taxon.objects.disable_mptt_updates():
                        ...
                        self.authority = record_taxon.authority
                        if record_taxon.rank.lower() in RANKS_NAMES.keys():
                            self.rank = RANKS_NAMES[record_taxon.rank.lower()]
                        self.status = record_taxon.status
                        if record_taxon.status == 'accepted':
                            self.is_valid = True
                        ...
                        for synonym in synonyms:
                            if synonym.status == 'accepted':
                                ...
                                if created_syn:
                                    ...
                                    if synonym.valid_name:
                                        syn.is_valid = True
                                    syn.valid_taxon = self
                        if record_taxon.status == 'accepted':
                            self.is_valid = True
                            self.valid_taxon = self
                        else:
                            self.is_valid = False
                            taxon_valid, created = Taxon.objects.get_or_create(name=record_taxon.valid_name.capitalize())
                            if created:
                                taxon_valid.update_taxonomic_tree()
                        if rank == 'cls':
                            index_rank = list(RANKS_NAMES.keys()).index('class')
