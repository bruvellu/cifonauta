...
def create_location(request):
    ...
    if serializer.is_valid():
        serializer.save()
        return Response(
            {"message": "Local adicionado com sucesso", "data": serializer.data}
        )
...
def create_authors(request):
    ...
    if serializer.is_valid():
        serializer.save()
        return Response(
            {"message": "Coautor registrado com sucesso", "data": serializer.data}
        )
...
def dashboard(request):
    ...
    context = {"is_editor": is_editor, "is_curator": is_curator}
...
def upload_media_step1(request):
    ...
    if request.method == "POST":
        ...
        if files:
            ...
            for file in files:
                ...
                if not extension:
                    messages.error(
                        request, f'Arquivo inválido: "{file.name}" não tem uma extensão'
                    )
    ...
    if Media.objects.filter(user=request.user, status="loaded").exists():
        ...
        messages.info(
            request, "Complete ou cancele o upload para adicionar outras mídias"
        )
        return redirect("upload_media_step2")
...
def upload_media_step2(request):
    ...
    if request.method == "POST":
        ...
        if action == "cancel":
            ...
            if form.is_valid():
                for media in medias:
                    specific_form = UploadMediaForm(
                        request.POST, request.FILES, media_author=person, instance=media
                    )
                ...
                form.send_mail(
                    ...
                    editors_user,
                    medias,
                    "Nova mídia para edição no Cifonauta",
                    "email_media_to_editing_editors.html",
                )
                messages.success(request, "As mídias foram enviadas para o editor.")
                messages.info(
                    request,
                    "Você ainda pode editá-las antes de serem submetidas para o curador.",
                )
        ...
        if metadata:
            ...
            for author in authors_meta:
                if author.strip() != "":
                    try:
                        authors.append(
                            Person.objects.filter(name=author.strip()).get().id
                        )
                    except:
                        messages.error(
                            request, f"O Co-Autor {author.strip()} não está cadastrado."
                        )
...
def synchronize_fields(request):
    if request.GET.get("country_id"):
        ...
        data = {"states": list(query.values("id", "name"))}
    ...
    if request.GET.get("state_id"):
        ...
        data = {"cities": list(query.values("id", "name"))}
...
def editing_media_details(request, media_id):
    ...
    if request.method == "POST":
        ...
        form = EditMetadataForm(
            request.POST, instance=media, editing_media_details=True
        )
        ...
        if form.is_valid():
            ...
            if action == "submit":
                ...
                form.send_mail(
                    ...
                    author,
                    [media],
                    "Mídia enviada para revisão no Cifonauta",
                    "email_media_to_revision_author.html",
                )
                ...
                form.send_mail(
                    ...
                    curators_user,
                    media,
                    "Fluxo da mídia no Cifonauta",
                    "email_media_to_revision_curators.html",
                )

            if action == "submit":
                messages.success(
                    request,
                    f"A mídia ({media.title}) foi enviada para revisão com sucesso",
                )
            else:
                messages.success(
                    request, f"A mídia ({media.title}) foi salva com sucesso"
                )
                messages.warning(request, "Você ainda não a enviou para revisão")
...
def search_media(queryset, query):
    ...
    filtered_queryset = queryset.annotate(rank=search_rank).filter(
...
def editing_media_list(request):
    ...
    queryset = (
        Media.objects.filter(status="draft")
        .filter(taxa__in=curations_taxa)
        .distinct()
    ...
    if request.method == "POST":
        ...
        if action == "entries_number":
            records_number = number_of_entries_per_page(
                request, "entries_curadoria_media_list", request.POST["entries_number"]
            )
        else:
                ...
                if form.is_valid():
                    ...
                    for media in medias:
                        if media.status == "submitted":
                            messages.error(
                                request,
                                "Não é possível realizar ação em lotes de mídia já submetida para revisão",
                            )
                            return redirect("editing_media_list")
                        ...
                        if form.cleaned_data["status_action"] != "maintain":
                            if not media.title_pt_br or not media.title_en:
                                messages.error(
                                    request,
                                    "Não é possível submeter mídia com campos obrigatórios faltando",
                                )
                                return redirect("editing_media_list")
                    ...
                    error = execute_batch_action(
                        request, medias, person, "editing_media_list"
                    )
                    if error:
                        return redirect("editing_media_list")
                        ...
                        if "taxa_action" in form.cleaned_data.keys():
                            if form.cleaned_data["taxa_action"] != "maintain":
                                curations = Curation.objects.filter(
                                    taxa__in=form.cleaned_data["taxa"]
                                )
                        else:
                            taxa = Taxon.objects.filter(
                                media__id__in=media_ids
                            ).distinct()
                    ...
                    messages.success(
                        request, _("As ações em lote foram aplicadas com sucesso")
                    )
                else:
                    messages.error(
                        request, _("Houve um erro ao tentar aplicar as ações em lote")
                    )
            else:
    ...
    filter_form = DashboardFilterForm(
        query_dict, user_curations=curations, is_editing_media_list=True
    )
...
def my_media_details(request, pk):
    ...
    if request.method == "POST":
        ...
        if media.status == "submitted":
            messages.error(request, "Não foi possível fazer alteração")
            return redirect("my_media_details", pk)

        if modified_media and not modified_media.altered_by_author:
            messages.error(
                request,
                "Não é possível realizar mudanças em uma mídia com alterações pendentes.",
            )

            return redirect("my_media_details", pk)

        form = UpdateMyMediaForm(
            request.POST, instance=media, media_author=person, media_status=media.status
        )
        if form.is_valid():
            if media.status == "published":
                if modified_media:
                    if modified_media.altered_by_author:
                        if form.has_changed():
                            ...
                            messages.error(
                                request, "Mudança igual à versão publicada no site"
                            )
                            messages.warning(
                                request,
                                "Descarte a alteração pendente ou efetue uma alteração válida",
                            )
                            return redirect("my_media_details", media.pk)
                    else:
                        messages.warning(
                            request,
                            "Esta mídia tem alterações pendentes de um editor. Não é possível realizar alterações até que elas sejam revisadas pelo curador",
                        )
                        return redirect("my_media_details", pk)
                    ...
                    if form.has_changed():
                        new_modified_media = ModifiedMedia(
                            media=media, modification_person=person
                        )
                        form = UpdateMyMediaForm(
                            request.POST,
                            instance=new_modified_media,
                            media_author=person,
                            media_status=media.status,
                        )
                ...
                messages.warning(
                    request,
                    "As alterações serão avaliadas e podem ou não serem aceitas",
                )
            else:
    ...
    if modified_media:
        if not messages.get_messages(request):
            if modified_media.altered_by_author:
                messages.warning(
                    request,
                    "Esta mídia tem alterações pendentes. Clique no botão abaixo para ver as alterações. Se você fizer novas alterações, as anteriores serão sobrepostas",
                )
        ...
        if is_modification_owner and not modified_media.altered_by_author:
            ...
            messages.warning(
                request,
                f"Esta mídia tem alterações sua como editor. Para vê-las, <a href={url}>Clique aqui</a>",
            )
    elif media.status == "submitted":
        messages.warning(
            request,
            "Não é possível fazer alteração em mídias que estão submetidas para revisão",
        )
...
def my_media_list(request):
    ...
    if request.method == "POST":
        ...
        if action == "entries_number":
            records_number = number_of_entries_per_page(
                request, "entries_my_medias", request.POST["entries_number"]
            )
        else:
            media_ids = request.POST.getlist("selected_media_ids")
                ...
                if form.is_valid():
                    ...
                    if has_published_media:
                        messages.error(
                            request,
                            _(
                                "Não é possível realizar ação em lotes de mídia já publicada"
                            ),
                        )
                        return redirect("my_media_list")
                    ...
                    if has_submitted_media:
                        messages.error(
                            request,
                            _(
                                "Não é possível realizar ação em lotes de mídia submetida para revisão"
                            ),
                        )
                        return redirect("my_media_list")

                    error = execute_batch_action(
                        request, medias, person, "my_media_list"
                    )
                    if error:
                        return redirect("my_media_list")
    ...
    # Get media queryset
    queryset = (
        Media.objects.filter(user=person.user_cifonauta)
...
def manage_users(request):
    ...
    if request.method == "POST":
        ...
        if action == "enable-authors":
            ...
            not_authors = UserCifonauta.objects.exclude(
                Q(id__in=author_ids) | Q(id=request.user.id)
            )
            ...
            for user in not_authors:
                if user.uploaded_media.all():
                    messages.error(
                        request,
                        f'O usuário "{user.first_name} {user.last_name}" possui mídia relacionada',
                    )
    ...
    authors_queryset = UserCifonauta.objects.filter(is_author=True).exclude(
        id=request.user.id
    )
    ...
    users = [
        {
            ...
            "curation_ids": [
                str(curation.id) for curation in curations.filter(Q(editors=user.id))
            ],
        }
...
def revision_media_list(request):
    ...
    if request.method == "POST":
        ...
        if action == "entries_number":
            records_number = number_of_entries_per_page(
                request, "entries_revision_media", request.POST["entries_number"]
            )
        else:
            media_ids = request.POST.getlist("selected_media_ids")
            ...
            if media_ids:
                ...
                if form.is_valid():
                    ...
                    error = execute_batch_action(
                        request, medias, person, "revision_media_list"
                    )
                    if error:
                        return redirect("revision_media_list")
                    ...
                    if form.cleaned_data["status_action"] != "maintain":
                        ...
                        form.send_mail(
                            ...
                            medias,
                            "Mídia publicada no Cifonauta",
                            "email_published_media_author.html",
                        )
                        ...
                        form.send_mail(
                            ...
                            medias,
                            "Fluxo da mídia no Cifonauta",
                            "email_published_media_editors.html",
                        )
...
def revision_modified_media(request, media_id):
    ...
    if request.method == "POST":
        ...
        if action == "discard":
            form.send_mail(
                ...
                media.user,
                media,
                "Alteração de mídia no Cifonauta",
                "email_modified_media.html",
            ...
            form.send_mail(
                ...
                "email_modified_media.html",
                modification_accepted=False,
                modified_media_editors_message=True,
            )
        ...
        if form.is_valid():
            form = (
                ModifiedMediaForm(request.POST, instance=media, author_form=True)
                if modified_media.altered_by_author
                else ModifiedMediaForm(request.POST, instance=media)
            )
            form.save()
    ...
    form = (
        ModifiedMediaForm(instance=modified_media, author_form=True)
        if modified_media.altered_by_author
        else ModifiedMediaForm(instance=modified_media)
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if action == "publish":
                ...
                messages.success(
                    request, f"A mídia ({media.title}) foi publicada com sucesso"
                )
            else:
                messages.success(
                    request, f"A mídia ({media.title}) foi salva com sucesso"
                )
                messages.warning(request, "Você ainda não a publicou")

            return redirect("revision_media_list")
        else:
            messages.error(request, "Houve um erro ao tentar salvar a mídia")
        ...
        if action == "entries_number":
            ...
            if media_ids:
                form = BatchActionsForm(
                    request.POST, view_name="my_curations_media_list"
                )
                ...
                if form.is_valid():
                    ...
                    for media in medias:
                        if media.status != "published":
                            messages.error(
                                request,
                                "Não é possível realizar ação em lotes de mídias não publicadas",
                            )
                        ...
                        if media.modified_media.all().exists():
                            messages.error(
                                request,
                                "Não é possível realizar ação em lotes de mídias com alterações pendentes.",
                            )
                            return redirect("my_curations_media_list")
                        ...
                        if not is_media_curator:
                            messages.error(
                                request,
                                "Não é possível realizar ação em lotes de mídias que você não é curador",
                            )
                            return redirect("my_curations_media_list")

                    error = execute_batch_action(
                        request, medias, person, "my_curations_media_list"
                    )
                    if error:
                        return redirect("my_curations_media_list")
...
def my_curations_media_details(request, media_id):
    ...
    if request.method == "POST":
        ...
        if modified_media and (
            not is_modification_owner or modified_media.altered_by_author
        ):
            messages.error(
                request,
                "Não é possível realizar mudanças em uma mídia com alterações pendentes.",
            ...
            if is_only_media_editor:
                if modified_media:
                    ...
                    if form.has_changed():
                        ...
                        form = EditMetadataForm(
                            request.POST, instance=new_modified_media
                        )
                ...
                messages.success(
                    request, f"A mídia ({media.title}) foi alterada com sucesso"
                )
    ...
    if modified_media:
        if not messages.get_messages(
            request
        ):  # if it's not right after make changes to the media
            if is_modification_owner and not modified_media.altered_by_author:
                messages.warning(
                    request,
                    "Esta mídia tem alterações suas. Clique no botão abaixo para ver as alterações. Se você fizer novas alterações, as anteriores serão sobrepostas",
                )
            elif modified_media and not is_modification_owner:
                messages.warning(
                    request,
                    "Esta mídia tem alterações pendentes. Não é possível fazer novas mudanças até que ela seja revisada",
                )
        ...
        if is_modification_owner and modified_media.altered_by_author:
            ...
            messages.warning(
                request,
                f'Esta mídia tem alterações suas como autor. Para vê-las, <a href="{url}">Clique aqui</a>',
            )

        if not is_only_media_editor:
            url = reverse("revision_modified_media", args=[media_id])
            messages.info(
                request, f'Para revisar as alterações, <a href="{url}">Clique aqui</a>'
            )

    if media.status != "published":
        messages.warning(
            request,
            "Esta mídia ainda não pode ser alterada por aqui, apenas depois de publicada",
        )
...
def download_media(request, media_id):
    ...
    return FileResponse(
        open(media.file.path, "rb"), as_attachment=True, filename=filename
    )
...
def tour_add(request):
    ...
    context = {"form": form, "is_editor": is_editor, "is_curator": is_curator}
...
def get_tour_medias(request):
    try:
        ...
        curations = Curation.objects.filter(
            Q(editors=request.user.person.id) | Q(curators=request.user.person.id)
        )
        taxon_ids = []
        for curation in curations:
            taxon_ids.extend(curation.taxa.values_list("id", flat=True))
        medias = Media.objects.filter(
            taxa__id__in=taxon_ids, status="published"
        ).distinct()

        query = None

        if input_value:
            query = medias.annotate(title_lower=Lower(F("title"))).filter(
            ...
            "medias": [
                {
                    ...
                    "isRelated": True
                    if Tour.objects.filter(creator=request.user.id, media=media)
                    else False,
...
def home_page(request):
    ...
    try:
        ...
        tour_image = (
            tour.media.exclude(id=main_image.id).exclude(id=photo.id).order_by("?")[0]
        )
    except:
...
def media_page(request, media_id):
    ...
    media = get_object_or_404(
        Media.objects.select_related("location", "city", "state", "country"),
        id=media_id,
    )
    ...
    if not admin_form:
        try:
            ...
            admin_form = AdminForm(
                initial={"highlight": media.highlight, "tours": tour_list}
            )
        except:
            tour_list = Tour.objects.values_list("id", flat=True)
            admin_form = AdminForm(initial={"highlight": False, "tours": tour_list})
...
def tour_page(request, slug):
    ...
    authors, editors, taxa, locations, cities, states, countries, tags = extract_set(
        entries
    )
...
def press_page(request):
    ...
    photos = Media.objects.filter(
        highlight=True, is_public=True, datatype="photo"
    ).order_by("?")
    cover_photo = photos[0]
    photos = photos.exclude(id=cover_photo.id)[:8]
    # Videos
    videos = Media.objects.filter(
        highlight=True, is_public=True, datatype="video"
    ).order_by("?")[:8]
    context = {
        "photos": photos,
        "videos": videos,
...
def extract_set(media_list):
    ...
    locations = Location.objects.filter(
        id__in=media_list.values_list("location", flat=True)
    )
    ...
    countries = Country.objects.filter(
        id__in=media_list.values_list("country", flat=True)
    )
...
def build_url(meta, field, queries, remove=False, append=None):
    ...
    if remove:
        if field == "datatype":
            ...
            queries[field] = [
                q for q in queries[field] if not q["slug"] == meta["slug"]
            ]
    else:
