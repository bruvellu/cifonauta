...
class WordPressClient:
	...
	def checkCatName(self, cats):
		"""Get a list of categories names
		"""
		for c in self.getCategoryList():
			if c.name == cats:
				print
				print '\t%s encontrada! ID=%d' % (cats, c.id)
				print
				return True
				break
			else:
				print '\tnão é %s...' % c.name
	...
	def newCategory(self, catname):
		...
		idNewCat = int(self._server.wp.newCategory(self.blogId, self.user, self.password, catContent))
	...
	def newMediaObject(self, mediaFileName, upDate):
		...
		try:
			...
			mediaStruct = {
				...
				'overwrite' : xmlrpclib.boolean(upDate)
			}
			
			result = self._server.metaWeblog.newMediaObject(self.blogId, self.user, self.password, mediaStruct)
			return result['url']
